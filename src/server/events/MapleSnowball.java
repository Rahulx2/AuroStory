...
public class MapleSnowball {
    ...
    public void startEvent() {
        for (MapleCharacter chr : characters) {
            if (chr != null) {
                chr.getClient().getSession().write(MaplePacketCreator.rollSnowBall(false, 1, map.getSnowball(0), map.getSnowball(1)));
                chr.getClient().getSession().write(MaplePacketCreator.getClock(600));
            }
        }
        TimerManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (map.getSnowball(team).getPosition() > map.getSnowball(team == 0 ? 1 : 0).getPosition()) {
                    for (MapleCharacter chr : characters) {
                        if (chr != null)
                            chr.getClient().getSession().write(MaplePacketCreator.rollSnowBall(false, 3, map.getSnowball(0), map.getSnowball(0)));
                    }
                    winner = true;
                } else if (map.getSnowball(team == 0 ? 1 : 0).getPosition() > map.getSnowball(team).getPosition()) {
                    for (MapleCharacter chr : characters) {
                        if (chr != null)
                            chr.getClient().getSession().write(MaplePacketCreator.rollSnowBall(false, 4, map.getSnowball(0), map.getSnowball(0)));
                    }
                    winner = true;
                } //Else
                warpOut();
