...
public boolean messageOn() { 
    ...
    public boolean hasBannedMac() {
        ...
        if (macs.contains("7A-79-00-00-00-02")) {
            macs.remove("7A-79-00-00-00-02");//hamachi mac, we don't wanna blacklist this. TODO: whitelist in DB
        }
        boolean ret = false;
        int i = 0;
        try {
    ...
    public int login(String login, String pwd, boolean ipMacBanned) {
        ...
        Connection con = DatabaseConnection.getConnection();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT id, password, salt, banned, gm, pin, beta, greason, tempban FROM accounts WHERE name = ?");
    ...
    public int getLoginState() {
        ...
        try {
            ...
            loggedIn = false;
            e.printStackTrace();
            throw new RuntimeException("Everything sucks", e);
        }
    }
